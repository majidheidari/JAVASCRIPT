class ConnectionManager {
  ...
  connect(address) {
    ...
    this.conn.addEventListener('message', event => {
      console.log('Received message', event.date);
      this.receive(event.data);
    });
  }
  initSession() {
    const sessionId = window.location.hash.split('#')[1]; //vsetko po hash, 0 pred hash
    const state = this.locatTetris.serialize();
    if (sessionId) {
      this.send({
        type: 'join-session',
        id: 'sessionId',
  ...
  updateManager(peers) {
    const me = peers.you;
    const clients = peers.clients.filter(client.id => me !== client.id);
